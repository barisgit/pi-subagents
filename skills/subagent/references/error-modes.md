...
The slim schema is intentionally strict. Treat rejections as migration signals instead of adding compatibility fields.

## Validator and control rejections

| Rejection | Typical cause | Remediation |
| --- | --- | --- |
| `prompt-rejected` | Legacy `prompt` field was sent. | Use `message` for shared framing/resume or `task` inside `run`. |
| `tasks-rejected` | Legacy `tasks` array was sent. | Rename to `run` and ensure each item is a `Task`. |
| `dropped-field-rejected` | A removed field such as `model`, `cwd`, `metadata`, or `control` was sent. | Remove the field; move policy to agent config or text into `task`/`message`. |
...
| `additional-property-rejected` | A key outside the slim schema was present. | Delete the extra key or move the information into `task`/`message`. |

## Remediation checklist

When a call fails, compare it to the canonical shape in `../SKILL.md`, then check `migration.md` for old-to-new rewrites. Prefer a small explicit `run` call over recreating legacy management or workflow fields.
