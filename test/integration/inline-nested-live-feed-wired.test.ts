...
describe("inline nested live feed compact wiring", () => {
	it("renders a running child through the single compact path without the plain spawn line", () => {
		...
		writeRun(ids[1]!, { parentRunId: ids[0], agent: "fixer", label: "live child", tokens: 1200, events: [tool("read", { path: "/Users/example/live.txt" })] });
			...
			details: {
				...
				results: [{
					...
					progress: {
						...
						status: "running",
						task: "parent task",
						recentTools: [{ tool: "subagent", args: childTask, rawArgs: { agent: "fixer", task: childTask, label: "live child" }, endMs: Date.now() }],
						recentOutput: [],
						toolCount: 1,
						tokens: 0,
						durationMs: 100,
	...
	it("renders a terminal child through the multi compact path without the plain spawn line", () => {
		...
		writeRun(ids[3]!, { parentRunId: ids[2], state: "complete", agent: "explorer", label: "done child", tokens: 512, startedAt: 1_000, endedAt: 2_250, events: [tool("bash", { command: "echo done" })] });
			...
			details: {
				...
				results: [{
					...
					progress: {
						...
						status: "completed",
						task: "parent task",
						recentTools: [{ tool: "subagent", args: childTask, rawArgs: { agent: "explorer", task: childTask, label: "done child" }, endMs: 2_250 }],
						recentOutput: [],
						toolCount: 1,
						tokens: 0,
						durationMs: 1_250,
