...
function writeRunRecord(dir: string, id: string): void {
	...
	const records = [
		...
		{
			type: "message",
			timestamp: "2026-05-20T00:00:00.110Z",
			message: {
				role: "assistant",
				content: [{ type: "tool_use", id: `${id}-read`, name: "read", input: { path: "/abs/a.ts" } }],
			},
		},
		{
			type: "message",
			timestamp: "2026-05-20T00:00:00.120Z",
			message: {
				role: "assistant",
				content: [{ type: "tool_use", id: `${id}-bash`, name: "bash", input: { command: "echo hi" } }],
			},
		},
	];
	fs.writeFileSync(
...
describe("subagents-status uniform args rendering", () => {
	it("renders matched async and sync tool args identically", () => {
		...
		try {
			...
			const asyncLines = buildRightLines(
				theme,
				{ ownership: "foreign", run: asyncRun("async-a", asyncDir) },
				120,
			);
