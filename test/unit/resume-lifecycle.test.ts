...
import { __setStatusWriterWriteJsonForTest } from "../../status-writer.ts";
...
describe("resume lifecycle", () => {
	...
	it("resume reseeds the activity clock so the inactivity watchdog restarts", async () => {
		const h = setup(); const run = seedRun(tempDir!, 4444);
		...
		assert.ok(init!.lastActivityAt >= before, `lastActivityAt ${init!.lastActivityAt} should be >= resume time ${before}`);
		assert.ok(init!.lastActivityAt - init!.startedAt > 1000, "activity clock must not stay pinned to the original startedAt");
		assert.ok((init!.runnerHeartbeatAt ?? 0) >= before, "runner heartbeat must be reseeded on resume");
		assert.equal(init!.lastUpdate >= before, true, "lastUpdate must advance to the resume moment, not stay at startedAt");
	});
