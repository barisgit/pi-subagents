...
describe("mode-agnostic relabel", () => {
	it("uses a neutral default status-list heading", () => {
		const run: AsyncRunSummary = { id: "run-1", asyncDir: "/tmp/run-1", state: "running", mode: "single", startedAt: 1, steps: [{ index: 0, agent: "fixer", status: "running" }] };
	...
	it("uses a neutral not-found status message", () => {
		const result = inspectSubagentStatus({ id: "definitely-missing-run" }) as AgentToolResult<Details> & { isError?: boolean };
	...
	it("uses neutral interrupt handler messages", async () => {
		...
		const s = state(cwd);
		const pi = { events: { emit: () => {} }, getSessionName: () => undefined, setSessionName: () => {}, getAllTools: () => [] };
		const executor = createSubagentExecutor({
			pi, state: s, config: { parallel: { concurrency: 1 } }, asyncByDefault: false, tempArtifactsDir: cwd, childRegistry: new ChildAgentRegistry(), expandTilde: (v: string) => v,
			discoverAgents: () => ({ agents: [makeAgent("fixer", { model: "mock/test-model" })] }),
		} as never);
		const result = await executor.execute("id", { action: "interrupt", id: "all" } as never, new AbortController().signal, undefined, {
			cwd, hasUI: false, ui: {}, sessionManager: { getSessionId: () => "parent-session", getSessionFile: () => null }, modelRegistry: { getAvailable: () => [{ provider: "mock", id: "test-model" }] }, model: { provider: "mock" },
