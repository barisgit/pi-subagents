...
`,
	...
	it("keeps an empty async workflow group running via its statusless lifecycle marker", () => {
		...
		assert.equal(
			readRunViewForEntry(entry, entries)?.state,
			"running",
			"mutant: async-status must not synthesize an empty running workflow group as complete",
		);
		assert.equal(
			runViewFromRegistryEntry(entry, entries).state,
			"running",
			"mutant: subagents-status must not synthesize an empty running workflow group as complete",
		);
		...
		assert.equal(
			...
			false,
			"workflow group liveness marker must not write status.json",
		);
	});
	...
	it("does not let a stale running marker mask a failed workflow child", () => {
		...
		assert.equal(
			readRunViewForEntry(group, entries)?.state,
			"failed",
			"mutant: async-status running override must not mask a failed child",
		);
		assert.equal(
			runViewFromRegistryEntry(group, entries).state,
			"failed",
	...
	it("opens one statusless group and nests agent children under it", async () => {
		...
		assert.equal(
			fs.existsSync(path.join(group.runRecordDir, "status.json")),
			false,
			"workflow group must stay statusless",
		);
		assert.equal(
		...
		for (const child of children) {
			assert.equal(
				fs.existsSync(path.join(child.runRecordDir, "status.json")),
				true,
				"workflow children must persist their own status.json",
			);
		}
		...
		assert.equal(
			runViewFromRegistryEntry(group, entries).state,
			"running",
			"mutant: group status.json reclassifies the row instead of synthesizing from children",
		);
	});
	...
	it("records a raw workflow-level failure (after a successful child) as a failed child so the group synthesizes as failed", async () => {
		...
		assert.equal(
			runViewFromRegistryEntry(group, entries).state,
			"failed",
			"a raw workflow error must make the group synthesize as failed, not complete",
		);
	});
	...
	it("does not add a redundant synthetic child when a child already failed", async () => {
		...
		assert.equal(
			children.length,
			1,
			"the already-failed Z child is the only failure row; no redundant synthetic child",
		);
	...
	it("records an unknown-agent failure as a failed child so the group does not synthesize as complete", async () => {
		...
		assert.equal(
			runViewFromRegistryEntry(group, entries).state,
			"failed",
			"group with a failed child must synthesize as failed, not complete",
		);
	});
});
