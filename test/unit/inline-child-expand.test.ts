...
describe("inline child expansion", () => {
	it("renders a running child as a single rolled-up line and does not expand the grandchild", () => {
		...
		// A live nested child now uses the shared running glyph rather than an inline-only diamond.
		assert.match(lines[0]!, /◈ subagent: explorer · find files · ↳ 1 nested · 2 tools · 1.2kt/);
