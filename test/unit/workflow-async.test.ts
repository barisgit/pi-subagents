...
function setup(prefix: string, options: { asyncByDefault?: boolean; blockPrompt?: boolean; promptDelayMs?: (task: string) => number } = {}) {
	...
	const pi = { events: { emit: (channel: string, data: unknown) => events.emit(channel, data), on: (channel: string, handler: (data: unknown) => void) => { events.on(channel, handler); return () => events.off(channel, handler); } }, getSessionName: () => undefined, setSessionName: () => {}, getAllTools: () => [] };
...
describe("workflow async execution (VAL-ASYNC-WORKFLOW)", () => {
	it("params.async:true returns a running stub before the script finishes", async () => {
		...
		const result = await tool.execute?.("wf", { script: "await agent('A', 'slow');\nreturn 'done';", async: true }, new AbortController().signal, undefined, ctx as never);
		await promptStarted.promise;

		assert.equal(result?.isError, undefined);
		assert.equal((result?.content[0] as { text?: string }).text?.startsWith("Workflow running...\nState: running"), true);
	...
	it("asyncByDefault:true backgrounds without params.async", async () => {
		const { tool, ctx, promptGate, promptStarted } = setup("workflow-async-default-", { asyncByDefault: true, blockPrompt: true });

		const result = await tool.execute?.("wf", { script: "await agent('A', 'default async');" }, new AbortController().signal, undefined, ctx as never);
		await promptStarted.promise;
	...
	it("sync path surfaces the raw script value via content text and a real Details snapshot", async () => {
		...
		const result = await tool.execute?.("wf", { script: "return { value: 42 };" }, new AbortController().signal, undefined, ctx as never);
