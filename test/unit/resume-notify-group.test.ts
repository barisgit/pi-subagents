...
function createPi() {
	...
	const bus = {
		...
		emit: (channel: string, data: unknown) => inner.emit(channel, data),
	};
...
describe("resume notify group", () => {
	it("notify exactly once after dedupe eviction", () => {
		const h = createPi();
		const payload = {
			id: "resume-notify-once",
			runId: "resume-notify-once",
			agent: "fixer",
			success: true,
			state: "complete",
			summary: "done",
	...
	it("group refold rolls up accumulated child completions", () => {
		...
		for (const id of ["child-a", "child-b"]) {
			h.events.emit(SUBAGENT_ASYNC_RUN_COMPLETE_EVENT, {
				id,
				runId: id,
				parentRunId: "group-refold",
				notifyPolicy: "rollup",
				agent: id,
				success: true,
				state: "complete",
		...
		h.events.emit(SUBAGENT_ASYNC_COMPLETE_EVENT, {
			id: "group-refold",
			runId: "group-refold",
			notifyPolicy: "rollup",
			agent: "child-a,child-b",
			success: true,
			state: "complete",
			summary: "group",
			timestamp: Date.now(),
	...
	it("rollup dedups resumed child completions by runId keeping latest", () => {
		...
		assert.deepEqual(
			h.sent[0]!.message.details.children.map((child: { runId: string }) => child.runId),
			["child-a", "child-b"],
		);
	});
});
