...
export const OUTPUT_SYSTEM_INSTRUCTION = `Finish every run by ending your final message with your output wrapped in a trailing ${OUTPUT_OPEN}...${OUTPUT_CLOSE} block. The output block must be the LAST thing in your final message; earlier narration and fenced samples are ignored, but nothing may follow the final block. The trailing output block is the ONLY part of your work returned to the parent agent/caller. The parent agent/caller cannot see earlier narration or intermediate assistant messages. Make the block a self-contained, complete handoff: include every decision-relevant conclusion, finding, evidence/reference, change made, verification result, risk, blocker, and next step the parent needs. Include exact paths and commands when material. Do not reduce it to a one-line summary when useful information appeared earlier. Put a string there by default, or the exact shape requested; with a requested schema, preserve that exact shape and put the complete result in its available fields. Do not stop with prose only and do not call any finish tool. If you are waiting on an async/background run result, do not wait on it by default with sleep/status loops; Pi will send a completion or needs-attention message and trigger a new turn. Continue independent work or stop if blocked on that result. Use status/sleep checks only when immediate inspection is genuinely necessary.`;
