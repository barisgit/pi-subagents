...
- `extension-runtime.ts` — `registerSubagentExtension(pi)` activation entry: creates session-scoped `SubagentState`, `ChildAgentRegistry`, executor, host API, role manager, tools, commands, renderers, event listeners, widget client, and lifecycle cleanup.
...
## Design
`extension-runtime.ts` is the composition root. Each extension activation builds an isolated runtime around the live `pi` instance: state maps/timers, `createSubagentExecutor`, `ChildAgentRegistry`, `createAsyncJobTracker`, persona-dir registry, slash bridges, prompt-template bridge, notification/rendering surfaces, and `RootRoleManager`. Host and child sessions are intentionally asymmetric: host activations pin `currentPi`, install global cleanup/unsubscribe hooks, and publish a host API; child activations register a scoped child API and avoid process-global side effects that would clobber the host.
...
7. `before_agent_start` appends the active root role system prompt only in root sessions; `session_shutdown` tears down listeners, timers, widgets, slash bridges, prompt bridge, snapshots, animation state, async jobs, and root role state.
