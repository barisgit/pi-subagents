...
- `live-session-relay.ts` — listener-only process-global relay for live child `AgentSession` publications plus the host-activation-owned `LiveSessionDirectory`, which tracks per-run sessions and per-tool live progress by subscribing to each observed session, validates partial results via `dashboardPartialResult`, and forwards lifecycle events to an optional activation-owned display observer; the hub stores no sessions/history and the directory releases sessions on replacement, unpublish, and disposal.
...
Runtime flow: dispatch/runtime code reads `runtime-env.ts` for depth/nested-delegation guards, `runtime-paths.ts` for run/artifact temp roots, `utils.readStatus` for safe persisted status reads and sleep-aware stale reconciliation, `artifacts.ts` for artifact paths/writes/cleanup, `control-policy.ts` for needs-attention state (including engaged network waits), `formatting.ts` for stable presentation strings without importing surfaces, and `live-session-relay.ts` to deliver created child sessions to the current host observer without changing execution ownership or persistence.

## Integration

Primary consumers are higher layers: `dispatch/*` orchestrates child runs using shared agent configs, skills, cwd/status helpers, artifacts, env guards, and formatting; `runtime/*` uses root role selection and agent discovery; `state/*` can format/parse low-level status without depending on surfaces; `surfaces/*` reuses/re-exports shared formatters. `persona-registry.ts` is the bridge for other extensions to register internal persona dirs through Pi events, `current-pi.ts` is the guarded escape hatch for lifecycle-spanning callbacks that must call the live Pi host, and `live-session-relay.ts` bridges fresh child module instances to the host-owned dashboard session directory.
