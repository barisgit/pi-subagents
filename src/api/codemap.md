...
## Design
The API surface is event-published rather than imported directly by consumers. `createHostSubagentApi()` builds a stable `SubagentExposedAPI` object for root sessions with three public members: `spawnRaw(input)`, `list(options)`, and `lineage()`. `registerChildSessionApi()` publishes the same shape inside child sessions but intentionally stubs `spawnRaw` and `list` because nested raw spawning is unsupported by the in-process executor.
...
## Flow
1. Host activation calls `createHostSubagentApi({ pi, executor, config, state, getRegisteredPersonaDirs, discoverAgents })`.
2. The host API immediately emits `SUBAGENT_EXPOSE_API_EVENT` with `spawnRaw`, `list`, and `lineage`, plus `SUBAGENT_LINEAGE_EVENT` with provisional host lineage.
