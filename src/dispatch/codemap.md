...
- `in-process-executor.ts` runs children through the host `AgentSession` — async children run IN-PROCESS, not as subprocesses. `worktree.ts` is the ONLY `child_process` spawn in the codebase (git worktree add/remove). `startChildAgent` (the single chokepoint all paths funnel through) acquires one leaf permit from `leaf-concurrency.ts` before prompting and releases it when the child settles.
