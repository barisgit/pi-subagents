...
3. `render-widget.ts` orders jobs newest-first with attention pinned, nests child rows, hides workflow children under workflow group rows, renders metadata-aware workflow names and declared phase progress, and drives animation only while live non-lost jobs exist.
