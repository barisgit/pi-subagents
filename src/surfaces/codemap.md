...
- `subagents-status.ts` is the fullscreen dashboard component. It owns TUI lifecycle, paneOverlay integration, refresh cadence, selection/collapse state, split sizing, scope controls, per-open listed-live-session subscriptions, transcript-global tool/thinking display modes resolved from Pi keybindings, render-time partial-tool progress reads from the shared `LiveSessionDirectory`, and the per-overlay live-render cache. Collapsed mode keeps detail focus while Left/Right navigates visible runs, advertises that action in the footer, and renders each immediate-parent/current-run breadcrumb segment in its own agent color; expanded mode retains pane-focus arrows and its existing title. Selection changes render immediately from a bounded live tail (last 12 logical groups, preserving adjacent tool results) or validated persisted preview, then promote only a stable selection to its full transcript.
...
- `dashboard-detail-renderer.ts` renders right-pane details from `LiveRun`: retained owned sessions use their own Pi tool definitions and event-stream partial tool results, while foreign/reloaded/nested sessions use typed persisted messages plus the activation's all-tools renderer catalog. Both paths share Pi's native message/bash/tool component pipeline, dashboard tool/thinking display modes, ordered multi-step rendering, width-and-display-revision cache invalidation, and terminal-control sanitization. Preview wrappers alias the canonical session `cacheKey`, so promotion reuses full-session render and pending-tool state; `Loading transcript…` covers the brief no-preview fallback. An activation-owned pending-tool component store preserves renderer state, exact elapsed timing, and renderer-owned intervals across dashboard closes after first render; per-open rendered-line caches remain separate. Unusable persisted data retains the compact transcript fallback.
...
4. Foreground and slash results use `render-result.ts` for compact/expanded cards. It guards malformed details, rebuilds width-adaptive composition from each component render width, renders progress/tool history/tokens/duration/sparklines, delegates nested child lookup/rendering to `render-inline.ts`, and manages result animation timers.
5. `/subagents-status` lazily ensures the host renderer catalog, then opens `SubagentsStatusComponent` with Pi's live keybindings and tool-expansion UI controller. The component derives rows through `dashboard-run-source.ts`/`dashboard-row-model.ts`; each reload snapshots live sessions per run from the host `LiveSessionDirectory` (the activation registry answers direct children when the directory has none) and reconciles the component's per-open subscriptions and live-render cache against every listed run, including disk-derived foreign nested ones. Selection changes cancel the pending settle timer and advance a generation; after the default 50 ms stable window, live sessions promote to full messages and persisted sessions call `RunMessageReader.read()`. Cancellation prevents stale promotion before a synchronous read begins; it does not abort an in-progress read. Subscriptions invalidate bounded live tails and request renders; tool progress and pending native-tool component updates are owned by activation-level session event subscriptions. Disposal clears settle/refresh/action timers, unsubscribes live sessions, clears per-open caches, and disposes the overlay, while activation-owned pending tool components survive close/reopen until final result or session cleanup. Unavailable catalog or unusable messages fall back to the existing compact transcript.
...
- Uses shared utilities for agent discovery/colors, formatting, logger, current Pi resolution, and status file reads; the runtime injects the activation-owned `LiveSessionDirectory` (`shared/live-session-relay.ts`) as the dashboard's live-session and tool-progress source.
...
- `dashboard-detail-renderer.ts` — renders selected dashboard details through one native Pi component pipeline for retained, preview, or typed persisted messages; preview cache-key aliasing preserves renderer state, `Loading transcript…` covers missing previews, and compact workflow/transcript fallback remains available.
...
- `slash-commands.ts` — registers `/run`, `/parallel`, `/subagents-status`, argument completions, bg/fork parsing, and dashboard shortcut; dashboard session lookup prefers the host live-session directory with the activation registry as a direct-child fallback.
- `subagents-status.ts` — fullscreen status dashboard with immediate bounded live/persisted previews, generation-guarded default-50ms stable-selection promotion to full transcripts, live-tail invalidation, paneOverlay split panes, scoped run loading, per-open subscriptions/caches, complete timer/subscription disposal, refresh, collapsed horizontal run navigation and breadcrumb chrome, and selection.
