...
- `dashboard-detail-renderer.ts` renders right-pane details from `LiveRun`: retained owned sessions use their own Pi tool definitions and event-stream partial tool results, while foreign/reloaded/nested sessions use typed persisted messages plus the activation's all-tools renderer catalog. Both paths share Pi's native message/bash/tool component pipeline, dashboard tool/thinking display modes, ordered multi-step rendering, width-and-display-revision cache invalidation, and terminal-control sanitization. An activation-owned pending-tool component store preserves renderer state, exact elapsed timing, and renderer-owned intervals across dashboard closes after first render; per-open rendered-line caches remain separate. Unusable persisted data retains the compact transcript fallback.
...
5. `/subagents-status` lazily ensures the host renderer catalog, then opens `SubagentsStatusComponent` with Pi's live keybindings and tool-expansion UI controller. The component derives rows through `dashboard-run-source.ts`/`dashboard-row-model.ts`; each reload snapshots live sessions per run from the host `LiveSessionDirectory` (the activation registry answers direct children when the directory has none) and reconciles the component's per-open subscriptions and live-render cache against every listed run, including disk-derived foreign nested ones. Those subscriptions exist only to invalidate the cached tail and request renders (selected or not); tool progress and pending native-tool component updates are owned by activation-level session event subscriptions. Per-open subscriptions and rendered-line caches are torn down with the component, while pending tool components survive close/reopen and display-revision rebuilds until final result or session cleanup. Runs without a host session handle read stat-invalidated typed messages from disk. Unavailable catalog or unusable messages fall back to the existing compact transcript.
...
- `dashboard-detail-renderer.ts` — renders selected dashboard details through one native Pi component pipeline for retained or typed persisted messages, with compact workflow/transcript fallback.
...
- `subagents-status.ts` — fullscreen status dashboard component with paneOverlay split panes, scoped run loading, per-open reconciled listed-run live-session subscriptions with render-time `LiveSessionDirectory` progress reads (including disk-derived foreign nested runs), persisted native fallback, per-overlay caches, row rendering, refresh, collapse, and selection.
