...
- `subagents-status.ts` is the fullscreen dashboard component. It owns TUI lifecycle, paneOverlay integration, refresh cadence, selection/collapse state, split sizing, scope controls, selected-session subscriptions, and the per-overlay weak live-render cache.
...
- `dashboard-detail-renderer.ts` renders right-pane details from `LiveRun`: retained owned sessions use Pi's native message/tool/bash components (including ordered multi-step sessions), while foreign, reloaded, or unusable sessions retain the persisted transcript path; workflow groups still render metadata plus the ordered phase/script outline.
...
- `dashboard-detail-renderer.ts` — renders selected dashboard row details from retained live sessions with Pi components or from the persisted workflow/transcript fallback.
