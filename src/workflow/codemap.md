...
4. Child start increments `childIndex`, records phase/parallel metadata, emits a running placeholder, and dispatches the child. Child settlement replaces the placeholder with the compacted final `SingleResult` and emits an updated details snapshot.
...
6. Sync workflows await `runWorkflowScript()` and return the script value as text plus `emitter.snapshot()` details. Async workflows start `run()` in the background, immediately return a running async details payload, and later call `finishAsync()` or `failWorkflow()` with current phase tags. Child-session workflows remain sync unless `allowNestedAsync` is enabled; enabled nested async completion starts a new turn in the immediate parent session.
7. `workflow({ resume: id, script? })` reopens the SAME group in place (same id, directory, and journal; `resolveWorkflowResume` checks ownership and that nothing is live, and supersedes an earlier script failure), re-runs the saved (or corrected) script, and keys each `agent()` call by hash(role, task, schema, cwd) plus occurrence. Journaled results return without dispatch; started-but-unfinished calls reopen their own child run and session file (`resolveWorkflowChildResume`) and are prompted to continue; unreached calls dispatch fresh. Failure text carries the workflow id and a resume hint.
8. Any raw workflow-level error is caught by the tool, optionally recorded as a synthetic failed workflow child through `group.failWorkflow()`, and returned as an error result without invalid `Details` shape.
